...
export class AnnouncementsService {
  ...
  public async create(
    accountId: number,
    createAnnouncementDto: CreateAnnouncementRequest
  ) {
    const { images, ...announcement } = createAnnouncementDto;

    const newAnnouncement = await this.repository.create(
      accountId,
      announcement
    );
    ...
    for (const url of images) {
      const newImage = await this.repository.insertImage(
        newAnnouncement.id,
        url
      );
  ...
  public async findOne(id: number) {
    ...
    const instancedAnnouncement = plainToInstance(
      Announcement,
      findAnnouncement
    );
    instancedAnnouncement.account = plainToInstance(
      Account,
      instancedAnnouncement.account
    );
  ...
  public async remove(id: number) {
    const findAnnouncement = await this.findOne(id);
