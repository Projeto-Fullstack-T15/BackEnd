...
export class AnnouncementPrismaRepository implements AnnouncementRepository {
    ...
    public async create(account_id: number, data: CreateAnnouncementDto): Promise<Announcement> {
        try {
            const newAnnouncement = await this.db.announcement.create({ data: { ...data, account_id } });
    ...
    public async update(id: number, data: UpdateAnnouncementDto): Promise<Announcement> {
        try {
    ...
    public async getMany(): Promise<Array<Announcement & { account: Account }>> {
        try {
            ...
            throw new InternalServerErrorException("An error ocurred when tried to get announcements");
        }
    }

    public async insertImage(announcement_id: number, url: string): Promise<Image> {
        try {
            ...
            throw new InternalServerErrorException("An error ocurred when tried to insert an image");
        }
    }
    ...
    public async removeImage(id: number): Promise<void> {
        try {
            ...
            throw new InternalServerErrorException("An error ocurred when tried to delete image")
        }
    }
}